...
export const addUser = (req, callback) => {
  ...
  try {
    userMongoose.addUser(req, reqBody, (err, result) => {
      if (err) {
        callback(applicationUtils.genResponseObj(req.get('dc-language'), 'CM5000000', err.message, undefined))
      } else {
        callback(applicationUtils.genResponseObj(req.get('dc-language'), 'CM2000000', 'success', result))
      }
    })
  } catch (err) {
    logger.error('service addUser Unhandled Exception: ' + err);
    callback(applicationUtils.genResponseObj(req.get('dc-language'), 'CM5000000', err, undefined))
  }
};
    ...
    userMongoose.getUserById(req, req.params.userId, (err, result) => {
      if (err) {
        ...
        if (result) {
          ...
          callback(applicationUtils.genResponseObj(req.get('dc-language'), 'CM4090100', 'User not found', result))
        }
      }
    })
  } catch (err) {
